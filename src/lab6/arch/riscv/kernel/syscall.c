...
int64_t my_sys_read(uint64_t fd, char *buf, uint64_t len) {
    int64_t ret;
    struct file *file = &(current->files->fd_array[fd]);
    if (file->opened == 0) {
        printk("file not opened\n");
...
void do_syscall(struct pt_regs* regs) {
    switch (regs->reg17) {
        case SYS_WRITE:
            regs->reg10 = my_sys_write(regs->reg10, (const char *)regs->reg11, regs->reg12);
            break;
        case SYS_READ:
            regs->reg10 = my_sys_read(regs->reg10, (const char *)regs->reg11, regs->reg12);
            break;
        case SYS_GETPID:
            regs->reg10 = current->pid;
            break;
        case SYS_CLONE:
            ...
            break;
        default:
            Err("not support syscall id = %d", regs->reg7);
    }
    regs->sepc += 4;
}